...
## Retired 19/09/2026, with the Wheat board retext

Dean ruled the Wheat board's retext in as the shipped default the same day it was measured
(sheet v44: _"Harvest one of your buildings, even if it is 1 card short of full"_,
`wheatHarvestGate: 'nearFull'`, `wheatBarn: 0`). Its two decomposition arms and the full
proposal all became the base they used to be measured against:

| File                                  | What it was                                                            | Why it is here                                                                                                                                                                                                                                                                                                                           |
| ------------------------------------- | ---------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `wheat-gate-nearfull-v1.overlay.json` | HALF ONE of the retext: `wheatHarvestGate` alone moved to `'nearFull'` | Its result BECAME the shipped game (`rules.economy.noticeBoardPower.wheatHarvestGate` is `'nearFull'` on the base). As one leaf it now says nothing.                                                                                                                                                                                     |
| `wheat-no-barn-rider-v1.overlay.json` | HALF TWO: `wheatBarn` alone moved to 0                                 | Same. `wheatBarn` is 0 on the base.                                                                                                                                                                                                                                                                                                      |
| `wheat-board-retext-v1.overlay.json`  | Dean's proposal in full, both leaves at once                           | Its result BECAME the shipped game whole. The game it named is reproduced by `BASE_GAME_DATA` with no overlay at all. To replay the game it predates (the pre-19/09 Wheat board), use `overlays/notice-board-visit-v1.overlay.json` or any other overlay/testkit helper pinning `wheatHarvestGate: 'loaded'` and `wheatBarn: 1` by name. |
