...
/**
 * Added by ticket 06. v14 Wheat ships as printed rather than being redesigned,
 * on the explicit grounds that the sim should make the case rather than
 * judgement, so this is OBSERVE by construction: there is no number in the
 * design to fail against, only a question to answer with evidence.
 *
 * Two readings, because "hosting" could be vestigial in two different ways.
 * Coins off rivals' actions is the wage income Wheat's identity is supposed to
 * attract; play rate is whether anyone wants its cards at all.
 */
export const wheatIdentity: Assertion = {
  ...
  quote:
    ...
    "Wheat's low thresholds no longer pull traffic. The magnet is now the Draw Worker. " +
    "Re-examine what Wheat's cross-table identity is for.",
  source: 'docs/Unified Visit v14.md section 7.6',
  shape:
    "Wheat seats' mean coins earned off rivals' Worker uses against the set median; Wheat's mean " +
  ...
  remedy: NO_REMEDY,
  measure({ data, pooled }) {
    const wageBySuit = new Map<Suit, number[]>(SUITS.map((s) => [s, []]));
    for (const g of pooled.ended) {
      g.suits.forEach((suit, seat) => {
        wageBySuit.get(suit)?.push(g.wageCoinsBySeat[seat] ?? 0);
      });
    }
    const wageMeans = new Map([...wageBySuit].map(([s, xs]) => [s, mean(xs)]));
    ...
    return {
      ...
      headline:
        `Wheat earns £${num(wheatWage, 2)} per game off rivals against a set median of ` +
        `£${num(setMedian, 2)}; play rate ${pct(wheatPlay)} against a band of ` +
        `${pct(Math.min(...others))}-${pct(Math.max(...others))}`,
      detail: [
        `wage income by suit: ${[...wageMeans].map(([s, v]) => `${s} £${num(v, 2)}`).join('  ')}`,
