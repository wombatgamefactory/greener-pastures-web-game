 ...
 *
 * ## THE COINS (10/09/2026): TWO WEIGHTS, DELETED 16/09/2026
 *
 * `coinWorth` and `coinSpend` priced the commons-coins arm and then the Village
 * Store, and went with both. The lesson they recorded: they shipped at 3.5 on
 * the design's argument (K12, a Farmstead power "worth about two plain
 * actions") and were re-cut to 1.2 the same day on a measurement that found the
 * power returned about ZERO net over the action it displaced. A currency with no
 * price is a currency the bots mint and abandon.
 *
...
export const BALANCED: WeightTable = {
   ...
   */
  meepleSpend: 2.5,
  // ⛔ `coinWorth`, `coinSpend`, `mintStrands` and `balloon` were deleted on
  // 16/09/2026 with the coins, the Village Store and the balloons (R1, R2).
